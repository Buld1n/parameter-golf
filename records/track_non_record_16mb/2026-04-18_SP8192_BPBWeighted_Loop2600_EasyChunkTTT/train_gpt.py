...
class GPT(nn.Module):
    def __init__(self, h):
        ...
        self.bpb_weighted_loss = h.bpb_weighted_loss
    ...
    def forward(self, input_ids, target_ids):
        ...
        if (
            self.training
            and self.bpb_weighted_loss
            and hasattr(self, "_byte_weights")
        ):
            per_token_loss = F.cross_entropy(
                flat_logits, flat_targets, reduction="none"
            )
            byte_weights = self._byte_weights[flat_targets]
...
def train_model(h, device, val_data):
    ...
    if h.bpb_weighted_loss:
        base_model.register_buffer(
            "_byte_weights",
            val_data.base_bytes_lut.float().clamp_min(1.0),
            persistent=False,
        )
